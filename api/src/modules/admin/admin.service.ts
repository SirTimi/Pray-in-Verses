...
export class AdminService {
  ...
  async acceptInvite(dto: AcceptInviteDto) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        ...
        // normalize status if somehow missing in legacy rows
        if (!existing.status) updates.status = UserStatus.ACTIVE;
  ...
  async broadcastNotification(adminId: string, dto: CreateNotificationDto) {
    ...
    // Create the Notification first (explicit status)
    ...
    // Resolve recipients — **only ACTIVE** after backfill
